...
var url = process.env.MONGODB_URI || 'mongodb://localhost/YelpTravel_destinations'   // connect to MLAb, for deployment
// var url = 'mongodb://localhost/YelpTravel_destinations'                          // for local use
...
app.get("/destinations/:id/comments/:comment_id/edit", checkCommentOwnership, (req, res) =>{
  Comment.findById(req.params.comment_id, (error, foundComment) =>{
    if (error) res.redirect("back")
      ...
      req.flash("error", "Invalid request to edit comment!")
      res.redirect("back")
    }
  })
})
...
app.get("/login", function(req, res){
   res.render("login.ejs");    // message is the flash's message if req.flash(...) is triggered
});
...
app.get("/logout", function(req, res){
   req.logout();
   req.flash("error", "You are logged out!")      // set up flash message to display to user on the next page (destinations.ejs)
   res.redirect("/destinations");
});
...
function isLoggedIn(req, res, next){
    ...
    req.flash("error", "Please log in first!")      // set up flash message to display to user on the next page (login.ejs)
    res.redirect("/login");
}
